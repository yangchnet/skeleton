...
// The AuthzPolicyFunc type is an adapter to allow the use of ordinary
// function as AuthzPolicy mutator.
type AuthzPolicyFunc func(context.Context, *ent.AuthzPolicyMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AuthzPolicyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AuthzPolicyMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AuthzPolicyMutation", m)
	}
	return f(ctx, mv)
}
