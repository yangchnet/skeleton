...
func (tq *TenantQuery) sqlAll(ctx context.Context) ([]*Tenant, error) {
	...
	if query := tq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.tenant_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tenant_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tenant_users" returned %v for node %v`, *fk, n.ID)
			}
