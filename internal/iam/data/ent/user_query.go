...
func (uq *UserQuery) QueryPolicys() *AuthzPolicyQuery {
	query := &AuthzPolicyQuery{config: uq.config}
...
// WithPolicys tells the query-builder to eager-load the nodes that are connected to
// the "policys" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPolicys(opts ...func(*AuthzPolicyQuery)) *UserQuery {
	query := &AuthzPolicyQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPolicys = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withPolicys; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Policys = []*AuthzPolicy{}
		}
		query.withFKs = true
		query.Where(predicate.AuthzPolicy(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PolicysColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_policys
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_policys" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_policys" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Policys = append(node.Edges.Policys, n)
		}
	}
	...
	if query := uq.withBelong; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tenant_users" returned %v`, n.ID)
			}
			for i := range nodes {
